...
import { startWorkspaceTestingApp, fetchWorkspaceTestingAppStatus } from '../api'

export function WorkspaceTestingPane() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      try {
        ...
        const status = await fetchWorkspaceTestingAppStatus()
        ...
        const result = await startWorkspaceTestingApp()
        if (!cancelled) {
          if (result?.port) {
            setPort(result.port)
          } else {
