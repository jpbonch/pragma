...
export function buildWorkerPrompt(input: {
  ...
  const parts = [
    ...
    "## Preview file",
    "Before finishing, create a `preview.html` file in `outputs/$PRAGMA_TASK_ID/`.",
    "This file is a TOOL for the reviewer to interact with your work — NOT a report. It must contain ONLY functional UI elements. No headings, descriptions, explanations, or summaries of what you did.",
    "",
    "Common patterns:",
    "- Web app with a running service: a full-viewport iframe pointing at the service URL. Nothing else.",
    "- API with no frontend: a minimal form-based UI to hit each endpoint and display responses.",
    "- Static output (charts, data, generated site): the content itself, rendered directly.",
    "- CLI tool or library with no visual output: skip the preview file entirely.",
    "",
    "You can generate whatever HTML is needed for the specific task. The only hard rule: no prose.",
