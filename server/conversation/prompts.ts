...
export function buildWorkerPrompt(input: {
  ...
  const parts = [
    ...
    "## Preview file",
    "Before finishing, create a `preview.html` file in `outputs/$PRAGMA_TASK_ID/` so the reviewer can interact with your work directly in the review UI:",
    "- If you started a web app service: create an HTML page with a full-viewport `<iframe>` pointing at the service URL (e.g. `http://localhost:3001`). Use `style=\"width:100%;height:100vh;border:none;\"`.",
    "- If you built an API: create an HTML page with forms/buttons to test each endpoint. Include a response display area.",
    "- If you generated static output (charts, reports, a static site): bundle it as self-contained HTML.",
    "- This file is REQUIRED whenever you produce code changes. Do not skip it.",
