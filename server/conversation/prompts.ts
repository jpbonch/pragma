...
export function buildWorkerPrompt(input: {
  ...
  const parts = [
    ...
    "## Services & preview",
    "If your changes include a runnable app or server, start it for the reviewer:",
    "```",
    `${cli} task start-service --name "<name>" --command "<start cmd>" --cwd "<dir>" --port <port> --url <url>`,
    "```",
    `For example, for a Vite app: \`${cli} task start-service --name "frontend" --command "npm run dev -- --port 3001" --cwd "./code/myapp" --port 3001 --url "http://localhost:3001"\``,
    "",
    'Then create `outputs/$PRAGMA_TASK_ID/preview.html` — a TOOL for the reviewer to interact with your work, not a report. No prose, headings, or summaries.',
    "",
    "Patterns:",
